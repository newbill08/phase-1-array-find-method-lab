...
//Works
/* function superbowlWin(arrayRecords) {
    // for (const record of arrayRecords){} ---> go through each line and is represented by x
    //for (const x of arrayRecords){
        const winningRecord =  arrayRecords.find(arrayRecords => (arrayRecords.result === "W"))
      if (winningRecord){
...
//Another 
function superbowlWin(arrayRecords){
    storeSolved = arrayRecords.find(linebylinecompare => linebylinecompare.result === "W")
    return (storeSolved ? storeSolved.year : undefined )
